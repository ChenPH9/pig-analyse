...
const getHistoricalFinancialData = (): FinancialReport[] => {
  const historicalData: FinancialReport[] = [
    ...
    { year: 2026, quarter: 2, date: '2026-Q2', netProfit: 112.8, revenue: 885.2, debtRatio: 38.2, grossMargin: 26.8, cashFlow: 412.5, peRatio: 14.2, roe: 13.5, pbRatio: 7.8 },
    { year: 2026, quarter: 3, date: '2026-Q3', netProfit: 158.5, revenue: 1365.5, debtRatio: 36.8, grossMargin: 27.5, cashFlow: 588.5, peRatio: 13.5, roe: 19.2, pbRatio: 8.2 },
    { year: 2026, quarter: 4, date: '2026-Q4', netProfit: 192.3, revenue: 1725.8, debtRatio: 35.2, grossMargin: 28.2, cashFlow: 745.5, peRatio: 12.8, roe: 23.8, pbRatio: 8.8 },
  ];
...
const getHistoricalSowData = (): SowData[] => {
  ...
  for (const item of baseGrowth) {
    ...
    for (let month = 1; month <= 12; month++) {
      ...
      // 添加随机波动
      const randomFactor = 1 + (Math.random() - 0.5) * 0.025;
...
// 从公司公告获取母猪存栏数据
const fetchSowDataFromAnnouncement = async () => {
  try {
    console.log('正在从公司公告获取母猪存栏数据...');
    // 牧原股份官网投资者关系页面
    ...
    const response = await axios.get(irUrl, {
      headers: {
        ...headers,
        Referer: 'https://www.muyuanfoods.com/',
      },
      timeout: 10000,
    });
    ...
    // 尝试查找最新的母猪存栏公告
    const announcements = $('a:contains("生猪销售简报"), a:contains("销售情况"), a:contains("生产数据")');
    
    if (announcements.length > 0) {
      console.log('找到相关公告:', announcements.length, '个');
    }
    
  } catch (error) {
    console.log('公司公告数据获取失败:', error instanceof Error ? error.message : '未知错误');
  }
...
export const fetchFinancialData = async (): Promise<FinancialReport[]> => {
  ...
  try {
    ...
    // 获取基础历史数据
    let financialData = getHistoricalFinancialData();
...
export const fetchSowData = async (): Promise<SowData[]> => {
  ...
  try {
    ...
    // 尝试从公司公告获取
    await fetchSowDataFromAnnouncement();
